...
for u in "${ENABLE[@]}"; do
    ...
    # "spira-suites" from "spira-suites-prod.timer"). ctrl.sh check exits 0 when a
    # suspension is in force. If ctrl.sh is absent or fails, the unit is treated as not
    # suspended — a missing control tool is not a reason to refuse enabling everything.
    _cs="${u%"-${SPIRA_INSTANCE}.service"}"; _cs="${_cs%"-${SPIRA_INSTANCE}.timer"}"
    _cs="${_cs%.service}"; _cs="${_cs%.timer}"
    _suspended=0
    if [ -x "$SPIRA_HOME/ctrl.sh" ] && "$SPIRA_HOME/ctrl.sh" check "$_cs" >/dev/null 2>&1; then
        _suspended=1
    fi
    ...
    _changed=0; [ -n "${_CHANGED[$u]:-}" ] && _changed=1
