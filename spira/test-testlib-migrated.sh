...
# These 4 build their pass/fail counters a different way (pass()/fail(), or a hand-rolled
...
NEVER_OK_FAMILY="test-aeon-heartbeat.sh test-aeon-lease.sh test-concierge.sh test-suite-state.sh test-thrash-wall.sh"
#
...
NEEDS_TESTLIB_EXTENSION="test-canary.sh test-gate-diag.sh test-install-refusal.sh test-timer-templates.sh"
#
# Suites carrying unmigrated ok()/want()/nowant() awaiting migration strategy decision
# from sp-l2be6. These 380 suites were not in the scope of sp-29g55 (which handled
