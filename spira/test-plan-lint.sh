#!/usr/bin/env bash
#
# test-plan-lint.sh — the T0 test-plan lint's own fence: every suite declares
# its tier and UC coverage, every UC id it names exists, and a gap in T0-T3
# coverage is reported without failing the lint.
#
...
# its ROOT via git from its own location, so copying it (with
# suite-covers.sh) into a throwaway git repo is enough to isolate every
# assertion from the real, not-yet-migrated spira/ corpus.
#
...
lint() { env -i PATH="$PATH" HOME="$TMP" TERM=dumb bash "$ROOT/spira/plan-lint.sh" "$@" 2>&1; }

cat > "$ROOT/docs/test-plan/dispatch.md" <<'EOF'
# dispatch

* `UC-dispatch-01` [T1] — a claim writes a lease
* `UC-dispatch-02` [T2] — a stale lease is reclaimed
EOF
...
# from every docs/test-plan/*.md page is refused.
# ==========================================================================
...
rc_empty=0
env -i PATH="$PATH" HOME="$TMP" TERM=dumb bash "$EMPTY_ROOT/spira/plan-lint.sh" > /dev/null 2>&1 || rc_empty=$?
