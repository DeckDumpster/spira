...
SPIRA_TESTENV_REGISTRY SPIRA_GH_INTAKE_REPO SPIRA_GH_INTAKE_PRIORITY SPIRA_GH_INTAKE_TRUSTED SPIRA_GH_INTAKE_BEAD_REPO SPIRA_FLAKY_GH_REPO
...
spira_conf_defaults() {
    ...
    # WHICH GITHUB LOGINS ARE TRUSTED TO FILE WORK. Space-separated list; empty means nobody
    # is trusted and every issue goes to the untrusted digest. Set to the login returned by
    # `gh api user --jq .login` at install time, plus any other Spira installs you run.
    # Never uses author_association or collaborator status — those are not exclusive to the
    # operator (a colleague is a collaborator too).
    : "${SPIRA_GH_INTAKE_TRUSTED:=}"
...
export SPIRA_INSTANCE \
       ...
       SPIRA_TESTDB_BD SPIRA_TESTDB_DATA SPIRA_TESTDB_PORT SPIRA_INCIDENT_PRIORITY SPIRA_TESTENV_REGISTRY SPIRA_GH_INTAKE_REPO SPIRA_GH_INTAKE_PRIORITY SPIRA_GH_INTAKE_TRUSTED SPIRA_GH_INTAKE_BEAD_REPO SPIRA_FLAKY_GH_REPO \
