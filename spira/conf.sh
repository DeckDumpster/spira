...
SPIRA_LAND_MAXSEC SPIRA_LAND_GATE_RESERVE SPIRA_CERTIFY_PAR SPIRA_CERTIFY_SUITES SPIRA_VERDICT_TTL SPIRA_REBASE_ESCALATE_AT SPIRA_EVICTION_ESCALATE_AT SPIRA_VERDICT_WINDOW SPIRA_REMEDY_WINDOW SPIRA_PR_STALL_MINS SPIRA_DEFERRAL_ESCALATE_AT
SPIRA_LOOM_ADDR SPIRA_LOOM_BUDGET_MS SPIRA_LOOM_CACHE_S SPIRA_LOOM_BIN
SPIRA_BROKER_BIN SPIRA_BROKER_GH_CONFIG_DIR SPIRA_BROKER_GH_TOKEN SPIRA_CZAR_PASS_BIN SPIRA_QUEUE_WATCH_BIN SPIRA_SUPERVISE_BIN SPIRA_LANDING_PASS_BIN
...
spira_conf_defaults() {
    ...
    # THE RED-MAIN LABEL — marks a certified bead as the fix for main's currently-red
    # push gate. batch.sh holds the queue while main's last push gate is red (sp-221n8:
    # a batch can otherwise land on top of a red main and nothing then stops the next
    # batch doing the same); a certified member carrying this label is the one
    # exception, or a red main could never be fixed through the queue that broke it.
    : "${SPIRA_RED_MAIN_LABEL:=fixes-red-main}"
    ...
    # WHERE testenv-batch.sh APPENDS PER-SUITE TIMING ROWS. Each row is tab-separated:
    # run_id branch suite rc wall_secs bd_calls bd_ms mode. The __batch__ row holds the
...
export SPIRA_INSTANCE \
       ...
       SPIRA_TESTDB_BD SPIRA_TESTDB_DATA SPIRA_TESTDB_PORT SPIRA_INCIDENT_PRIORITY SPIRA_TESTENV_REGISTRY SPIRA_GH_INTAKE_REPO SPIRA_GH_INTAKE_PRIORITY SPIRA_GH_INTAKE_BEAD_REPO SPIRA_FLAKY_GH_REPO SPIRA_RELEASE_REPO \
       ...
       SPIRA_PLAN_LABEL SPIRA_INCIDENT_LABEL SPIRA_EXPRESS_LABEL SPIRA_RED_MAIN_LABEL SPIRA_NO_LOOP_LABEL \
       SPIRA_TOWN SPIRA_MIRROR SPIRA_EXPORTER SPIRA_DESIGN SPIRA_WIKI SPIRA_WIKI_HOOK SPIRA_DOLT_DATA \
       SPIRA_ALERT_GLOB \
