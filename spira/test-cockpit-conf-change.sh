...
# Common env for health.sh loop invocations.
common_env() {
    env -i PATH="$BASE_PATH" HOME="$TMP/home" TERM=dumb LC_ALL=C.UTF-8 \
...
printf '# test\n' > "$CONF_NOCHANGE"
_ec_nochange=0
timeout 4 common_env "$CONF_NOCHANGE" >/dev/null 2>/dev/null || _ec_nochange=$?
...
_touch_pid=$!
_ec_change=0
timeout 6 common_env "$CONF_CHANGE" >/dev/null 2>"$TMP/health_err.log" || _ec_change=$?
