...
#
# NO LONGER HOST-ONLY (G-12, sp-fe3ee). law-synth.sh used to live in the brain wiki
# checkout, which a container running only the harness does not have, and its absence made
# half of test-statute-projection.sh's assertions vanish silently behind an `if`. It now
# ships in the harness itself as spira/law-synth.sh, so this suite always has it; the `skip`
# below stays only as a COUNTED skip (TAP skip-all, exit 77) should that file ever go
...
# law-synth.sh ships in the harness itself (spira/law-synth.sh, sp-fe3ee) — no other
# checkout involved.
LAW_SYNTH_SH="$HERE/law-synth.sh"
[ -x "$LAW_SYNTH_SH" ] || skip "law-synth.sh not found at $LAW_SYNTH_SH — reported as skipped rather than silently folded into green"
...
# wrote a real, uncommitted regeneration over HEAD's 10-statute fixture page — sp-fe3ee.)
