...
BID2="$(bdq create "Test amend bead" -l "spira,plan,repo:fixture" --json 2>/dev/null \
...
BID3="$(bdq create "Test no-aeon bead" -l "spira,plan,repo:fixture" --json 2>/dev/null \
    | python3 -c 'import json,sys; d=json.load(sys.stdin); print((d if isinstance(d,dict) else d[0])["id"])' 2>/dev/null)"
...
BID4="$(bdq create "Test mailbox cleanup bead" -l "spira,plan,repo:fixture" --json 2>/dev/null \
    | python3 -c 'import json,sys; d=json.load(sys.stdin); print((d if isinstance(d,dict) else d[0])["id"])' 2>/dev/null)"
