    ...
    for br in "$@"; do
        ...
        if ! rebase_branch "$br" "$base" "$repo" "$name"; then
            ...
            if [ "${_rq_n:-0}" -ge "${SPIRA_REBASE_ESCALATE_AT:-3}" ]; then
                ...
                bead_reopen "$id" merge-conflict "$_reopen_note"
...
' "$(spira_home_repo)" 2>/dev/null)
    ...
    for br in $brs; do
        ...
        if [ "$confine_rc" = 1 ]; then
            bead_reopen "$id" not-confined "Reopened by sentinel: $gate_out"
...
print(d[0].get("status","-") if d else "-")' 2>/dev/null)"
        ...
        *)
            ...
            if [ "$wedged" = 1 ]; then
                ...
                bead_reopen "$id" merge-conflict "Reopened by sentinel: branch $br conflicts with $base. The branch carries $_rn_merge commit(s) from the previous session — rebase onto $base, resolve the conflict, and finish. A merge conflict is not an escalation."
