...
fayth_free() {           # fayth_free <fayth> [pool-remaining]
    ...
    # THE POOL CLAMPS LAST, because it is the outermost cap and the only one the personas
    # share.
...
# ======================================================================================
# API CAPACITY — the account's own five-hour window, and a second unrelated thing in this
# harness called "capacity".
#
# The other one: FAYTH_MAX_CONCURRENT is how many aeons may run at once. It has nothing to
# do with this one, which is whether the API will answer at all. The name collision is why
# the condition went unhandled for so long — `grep capacity` returned confident, irrelevant
# hits.
#
