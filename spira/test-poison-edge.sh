#!/usr/bin/env bash
#
# test-poison-edge.sh — edge cases for the poison valve: ask title correctness (sp-njwb),
# stale poison clear (sp-fx1p), thrash-requeue exemption (sp-pi3ez), and zero-attempt
# guard at POISON_AT=0.
#
...
# must have its spira-poison label removed automatically.
# --------------------------------------------------------------------------------------
echo
...
# --------------------------------------------------------------------------------------
# THRASH REQUEUES DO NOT POISON (sp-pi3ez).
# --------------------------------------------------------------------------------------
echo
thrash_event() {
    local id="$1" uuid
...
# --------------------------------------------------------------------------------------
# ZERO CHARGED ATTEMPTS SENDS NO MAIL (POISON_AT=0 EDGE CASE).
# --------------------------------------------------------------------------------------
echo
...
printf '\ntest-poison-edge.sh: %d passed, %d failed\n' "$pass" "$fail"
[ "$fail" -eq 0 ]
