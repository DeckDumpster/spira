...
install:
	...
	if [ ! -e "$$_rel" ]; then \
	    ...
	    for _b in loom broker czar-pass queue-watch spira-supervise spira-config; do \
	    ...
	    { printf 'commit %%s\n' "$$_sha"; \
	      for _b in loom broker czar-pass queue-watch spira-supervise spira-config panel; do \
