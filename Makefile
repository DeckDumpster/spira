...
install:
	...
	if [ ! -e "$$_rel" ]; then \
	    ...
	    for _b in loom broker czar-pass queue-watch spira-supervise spira-config tsd-write; do \
	    ...
	    { printf 'commit %%s\n' "$$_sha"; \
	      for _b in loom broker czar-pass queue-watch spira-supervise spira-config tsd-write panel; do \
