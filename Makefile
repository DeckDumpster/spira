...
install:
	...
	if [ ! -e "$$_rel" ]; then \
	    ...
	    for _b in loom broker czar-pass reconciler queue-watch spira-supervise spira-config tsd-write spira; do \
	    ...
	    { printf 'commit %%s\n' "$$_sha"; \
	      for _b in loom broker czar-pass reconciler queue-watch spira-supervise spira-config tsd-write spira panel; do \
