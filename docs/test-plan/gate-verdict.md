...
Primary test files (13 that exist — `test-gate-missing-cmd.sh` merged into `test-gate-preflight.sh` by sp-ajxg3; `test-gate-host-cores.sh` and `test-gate-fixture-diag.sh` deleted by sp-ztxbr, folded into `test-gate-base-evidence.sh`'s env-contract and full-output rows, the host-cores/governor rows already covered by `test-gate-unit.sh`; `test-gate-universal.sh` and `test-gate-metering.sh` added by sp-9ael3 for UC-05 and UC-25): `test-gate-base-evidence.sh`, `test-gate-base-selection.sh`, `test-gate-locks.sh`, `test-gate-metering.sh`, `test-gate-preflight.sh`, `test-gate-sweep.sh`, `test-gate-touched.sh`, `test-gate-tree.sh`, `test-gate-unit.sh`, `test-gate-universal.sh`, `test-gate-verdict.sh`, `test-reopen-queue-eject.sh`, `test-yield.sh`. Plus `test-soak.sh` (UC-21, T3): does not exist yet, deferred to nightly/batch per §7. No file is secondary to this area.
