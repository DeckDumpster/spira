...
> **2026-09-26: `test-suites-fixture-fault.sh` deleted** (sp-3x1f4; law-a-test-that-flips-is-deleted). It flipped in Concierge round 24c (2026-09-25): red in the full corpus, green on the re-run of the same tree. Its coverage — UC-25's fixture-fault classification row (75 with shared fixture, else red) and UC-27's "fixture-faults file one bead naming all borrowers", plus the D3 aggregate-filing case and G15's untested SPIRA_DB-restore property — is lost until a deterministic replacement is written. Section 3/7 already project one: T1 with a stub filer in place of the real Dolt fixture and `incident.sh`, which is almost certainly what made this suite's shared-fixture reset non-deterministic. Filed as sp-9vsk3.
...
Machine-readable declarations live in `docs/test-plan/test-infrastructure.toml` (schema: `test-plan/schema/catalogue.schema.json`), read by `spira/plan-lint.sh`.
