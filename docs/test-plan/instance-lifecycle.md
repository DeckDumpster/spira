...
| 32 | test-cadence-tool.sh::all | T4-ish, real systemd (gate.yml, host-run) | Split. DEMOTE-TO-T1 the drop-in text, span→OnCalendar mapping and refusals. Keep the real-systemd "never disarms" case (sp-dah) host-run in gate.yml, not acceptance. |
...
| 38 | test-uninstall.sh::all; test-install-rehearsal.sh::stray sweep | T2, 78; T4, gate.yml host-run | KEEP test-uninstall.sh, but share the render fixture and drop the git bare/clone used only for the landref bypass (use FORCE or artifact mode). Target ~20 s. |
| 39 | *(none)* | — | GAP G1 |
| 40 | test-install-rehearsal.sh::all; test-cadence.sh | T4, gate.yml host-run | test-install-rehearsal: KEEP, runs directly on gate.yml's own runner (real podman + user systemd), never nested inside the corpus's testenv container (which carries no podman) and never in acceptance.yml. test-cadence.sh: DELETE. Its formula check is covered by test-watchd-unit-name.sh (hermetic), and its live half by the rehearsal. |
...
| Cadence drop-in text | `cadence.sh` | test-cadence-tool (gate.yml host-run, podman + real systemd) | `_dropin_for <unit> <span> <reason>` → text. Only the "armed after apply" check needs systemd. |
...
| G12 | **The real-systemd properties run nowhere automated.** cadence "never disarms" (sp-dah), the install→uninstall byte-identical round trip, and the installed watcher names are all NA in CI; they are container- or host-only. | signals.tsv NA ×4; r1 "not_run_in_ci". | RESOLVED: test-install-rehearsal.sh and test-cadence-tool.sh run directly on gate.yml's own runner — real podman, real user systemd, no acceptance.yml involvement. They cannot run through the normal corpus step (testenv-batch.sh execs suites inside its own container, which carries no podman, so both would only ever SKIP there); a dedicated gate.yml step runs them host-side instead, reusing the same warm testenv image and the job's own compiled binaries. spira-cockpit.service's ExecStart is the one Rust-binary-backed unit install.sh does not already skip when the binary is missing (unlike loom/broker/landing-pass); the suite stubs it itself when no cargo build has run, so a bare checkout still proves the round trip. |
...
Out of the corpus container: the loom cargo-build + live-serve leg stays in acceptance.yml, release only (about 150 s today). test-install-rehearsal (~3 min, header) and the test-cadence-tool systemd half (~2 min) run host-side in gate.yml itself (gap G12) — never in acceptance.yml, and never a release blocker. test-host-unit-names and test-cadence.sh leave the suite set.
...
Use case ids, one line each, as `spira/plan-lint.sh` (sp-qu948's T0 lint) parses them — mechanically derived from the ID/Requirement/Tier columns of the section 2 table above; the table is the source of truth, this list exists only because the lint reads `* \`UC-<area>-NN\` [T<n>] — ...` and not a table row:

* `UC-instance-lifecycle-01` [T1] — `build.sh` builds loom, panel, broker, czar-pass and spira-supervise, in order, to the `SPIRA_*_BIN` paths. With no cargo it exits 0 and names the lost features. `--skip-build` never invokes cargo.
* `UC-instance-lifecycle-02` [T2] — `build-tarball.sh` produces `spira-YYYYMMDDTHHMMSSZ.tar.gz` from a clean checkout, unpacking to a same-named dir. MANIFEST records the source SHA, and `--name` pins stem and timestamp.
* `UC-instance-lifecycle-03` [T0] — The tarball ships every `@*_BIN@` binary that a non-optional unit's ExecStart references, executable. The build refuses without `--supervise-bin`, and no `sp-*` or `*.fixed` scratch files ship.
* `UC-instance-lifecycle-04` [T2] — `build-tarball.sh verify` accepts a MANIFEST whose commit exists in the source repo and refuses one that does not.
* `UC-instance-lifecycle-05` [T2] — `build-bd.sh --from-release` derives `beads_<ver>_<os>_<arch>.tar.gz` and refuses a checksum mismatch before unpacking.
* `UC-instance-lifecycle-06` [T2] — `release.sh cut` creates an annotated tag listing the bead ids landed since the previous tag, on the repo-map base ref (not hardcoded `main`). A no-op cut creates no tag, and `show` reads the tag back.
* `UC-instance-lifecycle-07` [T2] — `review.sh` records a ship/block verdict per release unit, files a finding bead on block, and never duplicates it. *(misfiled; price only)*
* `UC-instance-lifecycle-08` [T2] — `activate.sh` unpacks to a versioned, read-only release dir, atomically repoints `current` and keeps the previous release. `--dry-run` changes nothing.
* `UC-instance-lifecycle-09` [T1] — `activate.sh` prunes releases beyond `SPIRA_RELEASES_KEEP`, oldest first, and never prunes the `current` target.
* `UC-instance-lifecycle-10` [T2] — `activate.sh` refuses while this instance's aeons are live (naming them and `SPIRA_ACTIVATE_FORCE`), leaving `current` untouched. After the swap it daemon-reloads and restarts only non-aeon units.
* `UC-instance-lifecycle-11` [T2] — Rollback is re-activation of an already-unpacked release (unpack skipped), after which units execute the older code.
* `UC-instance-lifecycle-12` [T2] — `deploy.sh` resolves `latest`, refuses drafts, the already-current release, a DB-migration mismatch and a failed pre-deploy doctor, all before any disruptive step.
* `UC-instance-lifecycle-13` [T2] — `deploy.sh` drains first: a drain refusal blocks the deploy, and `--force` slays with `--keep-work --reopen`. It writes `SPIRA_PROD` into spira.conf before activate restarts services, then re-renders units with `SPIRA_PROD=releases/current/spira`.
* `UC-instance-lifecycle-14` [T2] — A post-activation health failure (doctor or skew) rolls back to the prior release and restores spira.conf. On a first deploy it removes `current` and re-renders on the checkout. It never re-enables ctrl-suspended units.
* `UC-instance-lifecycle-15` [T1] — A release installed from its tarball runs from a read-only tree: sentinel dispatches and loom serves without cargo, and `conf.sh` resolves `SPIRA_LOOM_BIN` to `bin/loom`.
* `UC-instance-lifecycle-16` [T1] — Root install refuses with exit 5 on each conflict (foreign harness owns the unit names, live `aeon.sh` for this home, gate lock held, instance mismatch, Dolt port serving another data_dir), naming the remedy and `SPIRA_INSTALL_CONFLICT_CONSIDERED`. A clean box passes.
* `UC-instance-lifecycle-17` [T1] — Root install refuses a git-checkout `SPIRA_PROD` (exit 2, override named) and a `CONFIGURE_PROD` without `conf.sh`. On an empty `SPIRA_RELEASES` it bootstraps `releases/bootstrap` with `current -> bootstrap`. Otherwise it refuses with exit 2 and names `activate.sh`.
* `UC-instance-lifecycle-18` [T2] — Root install exits 3 when installed but `ready.sh` fails. `--dry-run` prints each phase and mutates nothing.
* `UC-instance-lifecycle-19` [T1] — The unit installer refuses a checkout on the wrong branch or behind the landref, before writing any unit. It skips the check for artifact deploys and exact-tag detached HEADs, and `SPIRA_INSTALL_FORCE` overrides it.
* `UC-instance-lifecycle-20` [T1] — The unit installer refuses when another instance's config shares any of `SPIRA_RUN`, `SPIRA_DB`, `SPIRA_PROD`, `SPIRA_DOLT_DATA` or `SPIRA_TESTDB_PORT`, naming the key, the other instance and its file. It writes no unit on refusal.
* `UC-instance-lifecycle-21` [T1] — The unit installer refuses while this instance's aeons are live (before daemon-reload), and refuses when any ExecStart target is missing or lacks +x.
* `UC-instance-lifecycle-22` [T1] — Per-unit apply decision, as a table: changed → write, restart, count; unchanged → nothing, no daemon-reload, "unchanged"; masked → left, reported masked; operator-disabled → not re-enabled; ctrl-suspended → not enabled; world halted → `enable` without `--now`; `spira-aeon-*` → never restarted or enabled.
* `UC-instance-lifecycle-23` [T2] — A changed timer whose oneshot is mid-pass is applied only after the oneshot drains. After install, any enabled-but-inactive unit makes the installer exit non-zero.
* `UC-instance-lifecycle-24` [T1] — Rendering produces instance-suffixed `spira-*-<inst>` units whose timers target `Unit=spira-*-<inst>.service`. Shared units (cockpit-ensure, concierge, beads-push, dolt-beads) keep plain names. Watchers render as `spira-watch-<name>-<inst>.service`, never `@`. No `@PLACEHOLDER@` survives.
* `UC-instance-lifecycle-25` [T1] — Optional units are gated by what is present: loom and broker by their binary, mail-deliver by `inotifywait`, suites by `SPIRA_SELF_TEST` (derived from `.git` presence, overridable). A missing prerequisite marks the unit OPTIONAL, not installed and not enabled, with a note.
* `UC-instance-lifecycle-26` [T1] — The renderer refuses `@DOLT@` templates when dolt is absent and never emits an empty ExecStart. `SPIRA_PROD=` (explicitly empty) falls back to `SPIRA_HOME`, and cockpit-ensure derives from `dirname(SPIRA_PROD)`.
* `UC-instance-lifecycle-27` [T2] — Prune: a `spira-*-<inst>` unit absent from UNITS is disabled, stopped and removed. An orphaned watcher (row left the manifest) is disabled, and a manifest watcher is never pruned. Neither loop prunes the other's units.
* `UC-instance-lifecycle-28` [T2] — Legacy migration disables un-suffixed and `spira-watch@` units before their per-instance replacements are enabled. It never touches shared units, is silent on a clean box, and `--no-migrate-watchers` spares watchers.
* `UC-instance-lifecycle-29` [T1] — A non-prod install seeds `SPIRA_INSTANCE=<inst>` into `dirname(SPIRA_PROD)/spira.conf` once, preserving existing lines. A prod install does not write the file.
* `UC-instance-lifecycle-30` [T2] — `unit-ensure.sh` reinstalls missing or changed units (daemon-reload only then), is a no-op otherwise, and reports MISSING-TARGET without enabling.
* `UC-instance-lifecycle-31` [T0] — Unit-file invariants: every timer template is in UNITS (or OPTIONAL) and in ENABLE, and fires periodically. Restarting services have a reachable start limiter in `[Unit]`. WatchdogSec appears only on non-shell ExecStart. The verdict service has `TimeoutStartSec>=3600` and no CPUQuota.
* `UC-instance-lifecycle-32` [T1] — `cadence.sh` changes a timer's cadence via a revertible drop-in and never leaves it without a next elapse. It refuses unknown units, and verify refuses an empty sweep.
* `UC-instance-lifecycle-33` [T2] — `owned.sh list` emits every artefact kind in 5-field rows, with the unit rows equal to `install.sh --render` for that instance. `check` reports absent, present or drifted per artefact.
* `UC-instance-lifecycle-34` [T2] — `install.sh --diff` and `skew.sh units` return 0 when clean, 1 with DIFFERS or MISSING, and 3 when the installer is missing.
* `UC-instance-lifecycle-35` [T2] — `skew.sh check` returns 1 for NOT-LATEST or MANIFEST-MISMATCH, 0 "in effect", and 3 for missing releases, `current` or MANIFEST. In artifact mode it reads tags via gh.
* `UC-instance-lifecycle-36` [T2] — `skew.sh foreign` refuses branches that change a vendored harness copy outside the harness repo (exit 1). It exempts the harness's own repo and dev source, and returns 3 on conf failure.
* `UC-instance-lifecycle-37` [T2] — `skew.sh gap` counts commits behind the remote. `refresh` advances even under a live lease, swaps atomically (new inode) and stashes dirty files.
* `UC-instance-lifecycle-38` [T2] — `uninstall.sh` stops, disables and removes owned units, disables linger, and removes harness symlinks and session hooks. It is idempotent, refuses ambiguity when several instances are installed, reports strays (never deletes them), and honours `--purge` and `--dry-run`. A partial install uninstalls with exit 0.
* `UC-instance-lifecycle-39` [T2] — `uninstall.sh --purge-database` removes the DB only when the operator types back the bead count, and refuses on mismatch.
* `UC-instance-lifecycle-40` [T4] — A fresh-box install→ready→uninstall round trip on real systemd leaves the unit directory byte-identical to before.
* `UC-instance-lifecycle-41` [T1] — `world.sh stop` stops active work units (resolved instance-qualified name, with plain fallback). It never stops `spira-cockpit*`, `spira-loom*` or landing, and spares CI watchers unless `--hard`. `--hard` stops watchers in both name forms. On any failed stop it exits non-zero without printing STOPPED.
* `UC-instance-lifecycle-42` [T1] — `world.sh start` starts timers except those ctrl-suspended (with the reason printed) or systemd-disabled. It revives non-oneshot watchers stopped by `--hard` and leaves active ones alone.
* `UC-instance-lifecycle-43` [T2] — `world.sh drain` touches no unit and writes a stamp with a resume hint. `--timeout` exits 1 with NOT DRAINED / REMAIN GATED. `--deadline` slays with `--keep-work --reopen`. `resume` clears the stamp. Every aeon door carries both the drain and halt gates.
* `UC-instance-lifecycle-44` [T1] — `world.sh status` reports landing, live gate and landing workers from `/proc`, DRAINING, active CI watchers, and `svc: <result>` for a failed last run. It says "unknown" (not "inactive") for an unresolvable unit.
* `UC-instance-lifecycle-45` [T1] — `ctrl.sh`: a suspension needs a reason and an owner. check, list and resume work, and divergence reports suspended-but-running and masked-but-undeclared units.
* `UC-instance-lifecycle-46` [T2] — `acceptance-ci.sh` builds a scratch repo on `main` with a git identity and a repo-map, puts `~/.local/bin` on PATH, passes `XDG_RUNTIME_DIR`, and propagates `acceptance-run.sh`'s exit code.
* `UC-instance-lifecycle-47` [T1] — `acceptance-run.sh` runs install with `SPIRA_OPERATED=0`, runs the clone's own `ready.sh` (not masked by `\|\| true`), extracts bead ids robustly (GH#2950), proves landing by ancestry, and records a git note. It emits a single PASS/FAIL line.
