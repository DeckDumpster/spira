...
| UC-operator-channel-43 | `rule.sh enact` prints "Statute is live" only when the synth hook succeeds. A failing, missing or non-executable hook is non-zero and prints "NOT regenerated". `law-synth.sh` refuses an empty or under-half store unless `LAW_SYNTH_OVERRIDE=1`, and no-ops when `SPIRA_WIKI` is unset. `cockpit.sh statute` reports `?` without a wiki and MISMATCH/OK otherwise. | fail-closed, observability | T2 / CI |
...
| 43 | test-statute-projection.sh (12) | T3, 5 s | RESOLVED (sp-fe3ee): law-synth.sh now ships as spira/law-synth.sh, so the law-synth half no longer skips. KEEP at T2 |
...
1. **G-01 Double escalation of one mail backlog.** Every `watchd.sh notify` pass runs `_wd_notify_health` and `mail-health.sh` back to back (watchd.sh:1488-1494). When the delivery daemon is down and concierge mail is aged, both paths can mail Ryan about the same backlog. No test runs notify with a registered mailbox and a down daemon together. ESCALATED: fixing which alarm path is canonical means changing watchd.sh and/or mail-health.sh, outside this slice's ownership — filed as sp-rrzwh with the options, and mailed to the operator for the decision, rather than picked here.
...
10. **G-10 `mail-fence.sh`.** RESOLVED: docs/test-plan/safety-fences.md already claims test-mail-fence.sh as one of its primary files (its coverage map row 11 merges it into test-guards.sh, alongside the other bd-*/testenv guards it shares a payload-builder cluster with). It is a write-time fence over `SPIRA_MAIL`, the same shape as the bd-*-guard suites safety-fences owns, not an operator-channel behaviour — this plan does not claim it.
...
12. **G-12 Skips that are not reported.** test-concierge.sh (~40 % of its assertions) skips silently and still reports green. This is a reporting gap: skips must be counted in the standard record (T4/acceptance for the host-only parts). (test-statute-projection.sh's law-synth half no longer skips — sp-fe3ee.)
